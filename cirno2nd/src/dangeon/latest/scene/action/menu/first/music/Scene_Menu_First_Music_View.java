...
class Scene_Menu_First_Music_View extends Base_Scene_Menu_First____View {

	public Scene_Menu_First_Music_View(Base_View bv) {
		super(bv);
	}

	@Override
	protected MenuWindow createMenuWindow() {
		MenuWindow_SelfAdjusting window = new MenuWindow_SelfAdjusting(this) {
			@Override
			...
			private String getTitle(int index) {
				String line = SCENE.getContentName(index);
				if (SCENE.getSelectedContent().OBJECT instanceof Boolean) {
					return line;
				} else {
					int i = line.indexOf("　");
		...
		((Scene_Menu_First_Music) PARENT_SCENE).X = window.getViewX()
				+ window.getWidth();
		return window;
	}
}